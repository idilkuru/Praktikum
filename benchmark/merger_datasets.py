...
def merge_custom_datasets(datasets):
    ...
    for i, dataset in enumerate(datasets):
        if i == 0:
            ...
            subset = select_subset(dataset, ["arb", "arz"], 0.11)
            merged.extend(subset)
        elif i == 5:
            # Dataset 06: keep 45% random
            subset = select_subset(dataset, None, 0.11)
            merged.extend(subset)
        elif i == 6:
            # Dataset 07: keep 60% random
            subset = select_subset(dataset, None, 0.19)
            merged.extend(subset)
        else:
...
if __name__ == "__main__":
    ...
    output_path_custom = "../Data/merged_dataset_6000.jsonl"
